...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  if (!isOpen) {
    return (
      <div className="hidden md:flex flex-col items-center py-3.5 px-2 bg-[#050409] border-r border-zinc-800/70 w-16 h-full justify-between select-none shrink-0 z-20 overflow-hidden">
        {/* Top Controls */}
        <div className="flex flex-col items-center gap-3.5 w-full">
          {/* Nexuss AI Icon / Expand Toggle */}
          <button 
            onClick={onToggle}
            className="group relative w-10 h-10 rounded-xl bg-purple-600/10 hover:bg-purple-600/25 border border-purple-500/20 hover:border-purple-400/40 flex items-center justify-center transition-all cursor-pointer shadow-xs"
          ...
          <button
            onClick={onNewChat}
            className="w-10 h-10 rounded-xl bg-zinc-900 hover:bg-purple-600 text-zinc-300 hover:text-white border border-zinc-800 hover:border-purple-400/40 flex items-center justify-center transition-all shadow-sm cursor-pointer group"
            title="New Chat"
            aria-label="New Chat"
          >
        ...
        <div className="relative group cursor-pointer pt-2">
          <div className="relative w-9 h-9 rounded-full bg-gradient-to-tr from-purple-700 via-indigo-600 to-pink-500 flex items-center justify-center text-xs font-semibold text-white shadow-md ring-2 ring-purple-500/30 group-hover:ring-purple-400/60 transition-all">
            ES
            <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-emerald-400 border-2 border-[#0c0b14]" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      */}
      <aside className="absolute inset-y-0 left-0 z-50 md:relative md:inset-auto md:z-20 w-72 sm:w-76 bg-[#050409] border-r border-zinc-800/70 flex flex-col h-full max-h-full shrink-0 select-none shadow-2xl md:shadow-none overflow-hidden">
        ...
        <div className="px-3.5 pt-3.5 pb-2 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            <button
              onClick={onToggle}
              className="group relative w-8 h-8 rounded-lg hover:bg-purple-600/20 flex items-center justify-center transition-all cursor-pointer"
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="px-1.5 py-0.2 rounded-md bg-purple-500/15 border border-purple-400/30 text-[9px] font-semibold text-purple-300 uppercase tracking-wider">
                Pro
              </span>
            </div>
          </div>
        ...
        <div className="px-3 pt-1 pb-2 shrink-0">
          <button
            ...
            className="w-full relative flex items-center justify-between py-2 px-3 rounded-xl bg-gradient-to-r from-purple-950/40 via-zinc-900 to-purple-950/30 hover:from-purple-900/50 hover:to-indigo-950/50 border border-purple-500/30 hover:border-purple-400/60 text-zinc-100 font-medium text-xs sm:text-sm transition-all shadow-xs group cursor-pointer active:scale-[0.99]"
          >
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 rounded-lg bg-purple-500/20 border border-purple-400/30 flex items-center justify-center text-purple-300 group-hover:text-purple-200 group-hover:bg-purple-500/30 transition-colors">
                <Plus className="w-3.5 h-3.5 group-hover:rotate-90 transition-transform duration-200" />
              </div>
              <span className="tracking-tight">New chat</span>
            </div>
            <span className="text-[10px] text-zinc-500 group-hover:text-purple-300 font-mono transition-colors">
              ⌘N
            </span>
          </button>
        </div>
        ...
        <div className="px-3 pb-2 shrink-0">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-[#13111e]/80 border border-zinc-800/80 hover:border-zinc-700/80 rounded-xl pl-8 pr-7 py-1.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-purple-500/60 focus:bg-[#161324] transition-all"
            />
            {searchQuery ? (
              <button
        ...
        <div className="flex-1 overflow-y-auto px-2.5 py-2.5 space-y-4 min-h-0 text-xs">
          {filteredThreads.length === 0 ? (
            <div className="px-3 py-10 text-center text-zinc-500">
              <div className="w-10 h-10 mx-auto mb-2.5 rounded-xl bg-purple-950/30 border border-purple-900/30 flex items-center justify-center text-purple-400/60">
                      ...
                      return (
                        <div
                          ...
                          className={`group relative flex items-center justify-between px-3 py-2 rounded-xl text-left cursor-pointer transition-all border ${
                            isActive
                              ? 'bg-gradient-to-r from-purple-950/60 to-indigo-950/40 border-purple-500/50 text-purple-100 font-medium shadow-xs shadow-purple-950/40'
                              : 'bg-transparent border-transparent hover:border-white/[0.06] hover:bg-[#151224]/70 text-zinc-300 hover:text-zinc-100'
                          }`}
                        >
                          <div className="flex items-center gap-2 min-w-0 flex-1 pr-2">
                            <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                              isActive ? 'bg-purple-400 shadow-xs shadow-purple-400' : 'bg-zinc-600 group-hover:bg-purple-400/60'
                            }`} />
                      ...
                      return (
                        <div
                          ...
                          className={`group relative flex items-center justify-between px-3 py-2 rounded-xl text-left cursor-pointer transition-all border ${
                            isActive
                              ? 'bg-gradient-to-r from-purple-950/60 to-indigo-950/40 border-purple-500/50 text-purple-100 font-medium shadow-xs shadow-purple-950/40'
                              : 'bg-transparent border-transparent hover:border-white/[0.06] hover:bg-[#151224]/70 text-zinc-400 hover:text-zinc-100'
                          }`}
                        >
                          <div className="flex items-center gap-2 min-w-0 flex-1 pr-2">
                            <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                              isActive ? 'bg-purple-400' : 'bg-zinc-700 group-hover:bg-purple-400/60'
                            }`} />
                            <span className="truncate text-xs tracking-tight">
                              {thread.title}
                            </span>
        ...
        */}
        <div className="p-2.5 border-t border-zinc-800/80 bg-[#090812]/95 backdrop-blur-md shrink-0 mt-auto relative" ref={accountMenuRef}>
          {/* Account Card Trigger */}
          <div 
            onClick={() => setAccountMenuOpen(!accountMenuOpen)}
            className="flex items-center justify-between p-2 rounded-xl bg-[#131120]/80 hover:bg-[#1a162b] border border-white/[0.05] hover:border-purple-500/30 transition-all cursor-pointer group shadow-xs"
          >
            <div className="flex items-center gap-2.5 min-w-0">
              {/* Dual-ring Glowing Avatar */}
              <div className="relative w-8 h-8 rounded-full bg-gradient-to-tr from-purple-700 via-indigo-600 to-pink-500 flex items-center justify-center text-white text-xs font-semibold shrink-0 ring-2 ring-purple-400/40 group-hover:ring-purple-400/70 shadow-sm transition-all">
                ES
                <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-400 border-2 border-[#0c0b14]" />
              </div>
              ...
              <div className="min-w-0 truncate">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="px-1 py-0.2 rounded bg-gradient-to-r from-purple-600 to-indigo-600 text-[8px] font-bold text-white tracking-wider">
                    PRO
                  </span>
                </div>
            ...
            <div className="p-1 rounded-lg text-zinc-400 group-hover:text-purple-300 transition-colors">
          ...
          {accountMenuOpen && (
            <div className="absolute bottom-full left-2.5 right-2.5 mb-2 p-2 rounded-2xl bg-[#141124] border border-purple-500/30 shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-bottom-2 duration-200 z-50">
              ...
              <div className="space-y-1.5 text-xs">
                {onOpenUpgrade && (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-xl hover:bg-purple-950/40 text-purple-300 hover:text-purple-200 transition-colors text-left cursor-pointer"
                  >
                ...
                <div className="flex items-center justify-between px-2.5 py-2 rounded-xl border border-zinc-800/40 bg-black/40">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border border-transparent transition-colors duration-200 ease-in-out focus:outline-hidden ${
                      showDecorations ? 'bg-purple-600 border-purple-500' : 'bg-zinc-800 border-zinc-700'
                    }`}
                  >
                    <span
