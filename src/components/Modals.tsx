...
const ModalWrapper: React.FC<ModalWrapperProps> = ({ title, isOpen, onClose, children, icon }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="relative w-full max-w-lg rounded-2xl bg-[#14121e] border border-zinc-800 shadow-2xl p-6 overflow-hidden max-h-[85vh] flex flex-col"
      ...
      <div className="space-y-2.5 pt-2">
        {filtered.map(p => (
          <div
            ...
            className="p-3.5 rounded-xl bg-zinc-900/60 hover:bg-purple-950/30 border border-zinc-800/80 hover:border-purple-600/40 transition-all cursor-pointer group text-left"
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-semibold text-zinc-100 group-hover:text-purple-200 transition-colors">
                {p.title}
              </span>
    ...
    <ModalWrapper title="Upgrade to Nexuss Pro" isOpen={isOpen} onClose={onClose} icon={<Zap className="w-4 h-4" />}>
      ...
      <div className="pt-4 border-t border-zinc-800">
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold text-xs tracking-wide shadow-lg shadow-purple-600/30 transition-all cursor-pointer"
        >
      ...
      <div className="space-y-3 pt-2">
        {workflows.map((wf, idx) => (
          <div
            ...
            className="p-3.5 rounded-xl bg-zinc-900/60 hover:bg-zinc-800/80 border border-zinc-800 text-left cursor-pointer transition-all hover:border-purple-500/40 group"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-zinc-200 group-hover:text-purple-300">{wf.title}</span>
              <ArrowRight className="w-3.5 h-3.5 text-zinc-500 group-hover:text-purple-400 transition-transform group-hover:translate-x-0.5" />
            </div>
